...
#include <libraries/connection/rest_server/impl/rest_server_impl.hpp>
...
TEST_CASE("rest_server provides observable of sockets")
{
    ...
    const auto d = connection::rest_server::impl::create(connection::rest_server::server_config{.threads = 1}).subscribe_with_disposable(mock);
